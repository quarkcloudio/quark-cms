...
class ShopCategoryController extends QuarkController
{
    public $title = '商家分类';
    ...
    {
        $grid = Quark::grid(new ShopCategory)->title($this->title)->tree();
        $grid->column('id','ID');
    ...
    {
        $form = Quark::form(new ShopCategory);
        ...
        $form->tab('基本', function ($form) {
            ...
            $categorys[0] = '根目录';
            $getCategorys = ShopCategory::where('status',1)->get();
